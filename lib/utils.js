...
async function downloadServer (serverPath, serverName, version, triple, callback) {
  console.log(util.format('Updating %s binary...', serverName))
  ...
  writer.on('finish', () => {
    ...
    atom.notifications.addSuccess('PySearch update successful!')
    callback()
  })
  writer.on('error', () => {
    console.log('Unable to update binary')
    atom.notifications.addError(utils.format('Unable to update %s binary'), serverName)
  })
}
...
function checkPylsInstallation(serverPath, python) {
  return new Promise((resolve, reject) => {
    ...
    childProcess.on("exit", (code, signal) => {
      switch (code) {
        ...
        case 7: {
          const err = "Pyls is not installed."
          atom.notifications.addError(
            err,
            {
              dismissable: true,
...
exports.checkPylsInstallation = checkPylsInstallation
